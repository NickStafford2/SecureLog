...
void processBigArgument(const std::string& filename, const std::string& key, const std::string& type, const std::string& name){
    ...
    while (std::getline(file, line)){
        ...
        if(key == wordList[3] && type == wordList[5] && name == wordList[6] && previous_room != wordList[8]){
...
int main(int argc, char *argv[])
{
    ...
    else if (argc == 5 || argc == 7)
    {
        ...
        else if (argc == 7)
        {
            ...
            // processBigArguments(filename, token, (-E or -G)), name)
            processBigArgument(filename, std::string(argv[2]), std::string(argv[4]), std::string(argv[5]));
        }
    }
    else
    {
